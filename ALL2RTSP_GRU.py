...
plt.rcParams['figure.dpi'] = 250  # plt.show显示分辨率
font = {'family': 'serif',
...
for l in range(len(len_cumsum_datcsvhit)-1):
    for i in range(num_argin+num_delay+1+len_cumsum_datcsvhit[l],
        ...
        y_train_dat.append(datatconrtpn[i:i+num_argout])                             #形成前15/50个-->16th/51st的输入数据       
...
y_train_dat=np.reshape(y_train_dat,(y_train_dat.shape[0],y_train_dat.shape[1],num_featout))
#x_train_data,y_train_data=np.zeros((1,num_argin,num_feat)),np.zeros((1,num_argout,num_featout)) #本试验np初始因声发射不同样本，具有特殊性
#x_train_data=np.append(x_train_data,x_train_dat,axis=0)
#y_train_data=np.append(y_train_data,y_train_dat,axis=0)
print('训练集的输入、输出维数： ',x_train_dat.shape,y_train_dat.shape)
train_dataset = tf.data.Dataset.from_tensor_slices((x_train_dat, y_train_dat))
...
for t in range(num_times_tt):
    ...
    modelt.add(layers.GRU(128, activation='relu',return_sequences=True,
                          input_shape=(x_train_dat.shape[1],num_feat),
    ...
    savep=savedname+'model_1012_all2rtpn_{0:s}_{1:s}_{2:s}_{3:s}'
    ...
    for j in range(len(nameyt)):
        ...
        dattn1=datt1.values[:lentrtp+1,1]
        ...
        dattn2=datt2.values[:lentrtp+1,1]
        ...
        dattn3=datt3.values[:lentrtp+1,1]
        ...
        dattn4=datt4.values[:lentrtp+1,1]
        ...
        dattn5=datt5.values[:lentrtp+1,1]
        ...
        dattn6=datt6.values[:lentrtp+1,1]
    ...
    for l in range(len(testdasizcum)-1):
        for n in range(num_argin+num_delay+1+testdasizcum[l],testdasizcum[l+1]+1-(num_argout-1)):
            ...
            y_test_data.append(testdatartpn[n:n+num_argout])                             #形成前15个-->16th的输入数据
    ...
    y_test_data=np.reshape(y_test_data,(y_test_data.shape[0],y_test_data.shape[1],num_featout))
    ...
    for m in range(len(nameyt)):
        ...
        datapertest=np.append(np.zeros((num_argin)),y_predictm[:,num_stepord-1,0]*std_testout[m,0]+mean_testout[m,0])################<<<<<<<<<<<<<
        testdatapredicted=np.append(testdatapredicted,datapertest,axis=0)
        datapertest_raw=testdatartp[testdasizcum[m]+1:testdasizcum[m+1]+1]
        
        #plt.plot(range(1, num_argin + 2), testdataya1[testdasizcum[m]+1:testdasizcum[m]+num_argin+2], 'aqua',)
        plt.plot(range(1,len(datapertest_raw)+1),datapertest_raw,'aqua', label='raw')
        plt.plot(range( num_argin + 1, y_predictm.shape[0] +num_argin+ 1),y_predictm[:,num_stepord-1,0]*std_testout[m,0]+mean_testout[m,0],############<<<<<<<<<
                 'coral', label='predict')
        ...
        for i in range(testdasizcum[m]-m*(
            ...
            y_pdraw=y_test_data[i].reshape(num_argout,num_featout)
            y_pdpredict=y_predict[i]   #shape=(num_argout,num_featout)
            y_pdraw=y_pdraw[num_stepord-1,0]*std_testout[m,0]+mean_testout[m,0]                              #########<<<<<<<<<<<<
            y_pdpredict=y_pdpredict[num_stepord-1,0]*std_testout[m,0]+mean_testout[m,0]                     #########<<<<<<<<<<<<
        ...
        mape_pertest_history=np.reshape(mape_pertest_history,(-1,1,num_featout))###<<<<<<<<<<<<<<<
        ...
        mape_pertest_history1=np.reshape(mape_pertest_history1,(-1,1,num_featout))###<<<<<<<<<<<<<<<<<<<
        ...
        mape_pertest_history2=np.reshape(mape_pertest_history2,(-1,1,num_featout))#########<<<<<<<<<<<<
        ...
        mape_pertest_history3=np.reshape(mape_pertest_history3,(-1,1,num_featout))##########<<<<<<<<<<<<<<<<
        ...
        perpred_raw=np.reshape(perpred_raw,(-1,1,num_featout))############<<<<<<<<<<<<<
        perpred_pred=np.array(perpred_pred)
        perpred_pred=np.reshape(perpred_pred,(-1,1,num_featout))##########################<<<<<<<<<<<<<<<<<
        ...
        perpred_mape=np.reshape(perpred_mape,(-1,1))#num_featout=1   ######<<<<<<<<<<------------
    ...
    ax.plot(range(len(testdatartp)-1),testdatartp[1:],'b--',label='Ground truth value')
    ax.plot(range(len(testdatapredicted)-1),testdatapredicted[1:],'r',label='Predicted value')
    #ax.set_ylim(0,9)
    #ax.axvline(x=np.argmax(datn5), color='magenta', linestyle='--',linewidth = '1')
    #ax.xaxis.set_major_locator(plt.MultipleLocator(1))
    #linewidth = '1', label = "test", color=' coral ', linestyle=':', marker='|'
    ax1=ax.twinx()
    ax1.spines['right'].set_position(('axes', 1.0))
    #ax1.spines['right'].set_color('k')
    #ax1.tick_params(axis='y',color='k',labelcolor='k')
    #ax1.set_ylim(0,6000)
    #ax1.plot(range(len(datn1)),datn1,color='royalblue', linestyle='--',label='EA',linewidth = '1.5')
    ax1.plot(range(len(testdataraw)-1),2.037*testdataraw[1:],linestyle='-',label='Stress',linewidth = '1')
    ax.set_xlabel('                 Timestamps')
    ax.set_ylabel('RTSP / s')#,color='k'
    ax1.set_ylabel('        Stress / MPa ')#,color='k'
    plt.title('Contrast between Ground truth value and Predicted')
